...
$(function () {
  ...
  $(".documentFileLogsTrackingDatatable thead th").each(function (index) {
    ...
    if (index >= 0 && index < thLen - 1) {
      ...
      if (col_type == "text") {
        $("#searchable-row4").append('<th class="text-center"><input type="text" class="text-center filter-table"/></th>');
      }
      if (col_type == "no_filter") {
        $("#searchable-row4").append('<th class="text-center"></th>');
      }
      if (col_type == "name") {
        $("#searchable-row4").append('<th class="text-center"><input type="text" class="text-center filter-table" style="width:130px"/></th>');
      }
      if (col_type == "date") {
        $("#searchable-row4").append("<th class=\"text-center\"><input type=\"text\" class=\"text-center filter-table date-range\" style=\"width:100px\"/></th>");
      }
      if (col_type == "select") {
      ...
      if (col_type == "forignkey") {
        ...
        $("#searchable-row4").append(_th_select);
        $(".select-custom").select2();
      }
      ...
      if (index == 0) {
        $("#searchable-row4").append('<th class="align-middle text-center"></th>');
      } else {
        $("#searchable-row4").append('<th class="align-middle text-center"><div style="width: 80px"></div></th>');
      }
    }
  });
  currentColumns4.push({
